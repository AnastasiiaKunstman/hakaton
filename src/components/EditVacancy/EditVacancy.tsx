...
import {
  Box, TextField, Button, Typography, Grid, Stack, SvgIcon,
} from '@mui/material';
...
interface EditVacancyProps {
  ...
  pub_date: string;
  onSave: (editedVacancy: Vacancy) => void;
}
...
const EditVacancy: React.FC<EditVacancyProps> = ({
  ...
  const [editedVacancy, setEditedVacancy] = useState<Vacancy>(initialVacancy);

  const handleFieldChange = (fieldName: keyof Vacancy, value: string) => {
    setEditedVacancy({ ...editedVacancy, [fieldName]: value });
  };

  const handleSave = () => {
    // Сохранить отредактированную вакансию и передать ее на сохранение
    ...
    <Box maxWidth="xl" sx={{ p: '28px 0 71px' }}>
          ...
          >
            ...
            >
              <Grid xs>
                ...
                {/* <Dropdown sliceName="specialization" /> */}
              </Grid>
              <Grid xs>
                <Typography variant="caption" sx={{ marginBottom: '4px' }}>
                  Уровень
                </Typography>
                {/* <Dropdown sliceName="requiredEducationLevel" /> */}
              </Grid>
            </Grid>
          </Grid>
        </Grid>
        ...
        >
          ...
          >
            <Grid xs={12}>
              ...
              <Stack spacing={3}>
                {/* <Dropdown sliceName="required_skills" /> */}
              </Stack>
            </Grid>
      ...
      >
        <Box className="edit-buttons">
          <Button onClick={() => dispatch(deleteVacancy(id))} type="button" className="edit-button">
            Удалить вакансию
          </Button>
        ...
        <Box className="edit-buttons">
          ...
          <Button type="button" onClick={handleSave} className="publish-button">
            Сохранить
          </Button>
        </Box>
      </Box>
